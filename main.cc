...
class Submarine {
	...
	public:
		...
		Submarine(int newHealth, int newTorpedoes, int newSensors) : health(newHealth), torpedoes(newTorpedoes), sensors(newSensors) {
			if (newHealth > MAX_HEALTH or newHealth < 0) health = MAX_HEALTH; // Sanity checks
			if (newTorpedoes > MAX_TORPEDOES or newTorpedoes < 0) torpedoes = MAX_TORPEDOES;
			if (newSensors > MAX_SENSORS or newSensors < 0) sensors = MAX_SENSORS;
		}
		// Accessor Methods
		int get_health() const { return health; }
		int get_torpedoes() const { return torpedoes; }
		int get_sensors() const { return sensors; }

		// Mutator Methods
		void set_health(int newHealth) {
			if (newHealth > MAX_HEALTH or newHealth < 0) health = MAX_HEALTH;
			else health = newHealth;
		}
		void set_torpedoes(int newTorpedoes) {
			if (newTorpedoes > MAX_TORPEDOES or newTorpedoes < 0) torpedoes = MAX_TORPEDOES;
			else torpedoes = newTorpedoes;
		}
		void set_sensors(int newSensors) {
			if (newSensors > MAX_SENSORS or newSensors < 0) sensors = MAX_SENSORS;
			else sensors = newSensors;
		}
...
int main() {
	...
	for (int i = 0; i < subs.size(); i++) { //Repeat the setup for each player
		...
		subs.at(i).location = starting_location; //Save the location 
		subs.at(i).history.push_back(starting_location); //First entry in the history
		pause();
		clear_screen();
	}
	...
	while (true) { 
		...
		pause();         
		clear_screen();
		print_map(subs.at(current_player).location);
		cout << "=== STATUS ===\n";
		...
		cout << YELLOW  << "LOCATION   = " << subs.at(current_player).location.row << subs.at(current_player).location.column << endl;
		cout << RESET;
		...
		while (true) {
			...
			if (abs(subs.at(current_player).location.row - new_location.row) > 1 || abs(subs.at(current_player).location.column - new_location.column) > 1) {
				cout << "Sorry, that position is more than one space away.\n";
				continue;
			}
			cout << "New location accepted.\n";
			subs.at(current_player).location = new_location; //Update location 
			subs.at(current_player).history.push_back(new_location); //Add it to the history
			break;
		}
		...
		while (true) {
			...
			if (choice == 4 && !check_near(subs.at(current_player).location,RESUPPLY)) {
				cout << "Sorry, you are not near a resupply station.\n";
				continue;
			}
			if (choice == 5 && !check_near(subs.at(current_player).location,REPAIR)) {
			...
			if (choice == 1) { //FIRE TORPEDOES
				while (true) {
					...
					for (int i = 0; i < subs.size(); i++) {
						int y_dist = abs(target.row - subs.at(i).location.row);
						int x_dist = abs(target.column - subs.at(i).location.column);
